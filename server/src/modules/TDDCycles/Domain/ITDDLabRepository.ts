...
export interface ITDDLabRepository {
    ...
    obtainRunsOfLog(owner: string, repoName: string): Promise<any>;
    obtainJobsOfACommit(owner: string, repoName: string, jobId: number, attempt: number): Promise<any>;
    getRunsOfLogIds(owner: string, repoName: string): Promise<[string, number][]>; 
    getJobsDataFromLog(owner: string, repoName: string, listOfCommitsWithActions: [string, number][]): Promise<Record<string, JobDataObject>>;
